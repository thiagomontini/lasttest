...
def extract_component(pixel_region, visited, px, py):
    ...
    while not queue.empty():
        ...
        visited[x,y] = True
        pixels[x,y] = pixel_region[x,y]
        min_x = min(min_x, x)
        min_y = min(min_y, y)
        max_x = max(max_x, x)
        max_y = max(max_y, y)

        if x > 0 and pixel_region[x-1,y][3] != '\x00' and not visited[x-1,y]:
            queue.put((x-1,y))

        if x < pixel_region.w-1 and pixel_region[x+1,y][3] != '\x00' and not visited[x+1,y]:
            queue.put((x+1,y))

        if y > 0 and pixel_region[x,y-1][3] != '\x00' and not visited[x,y-1]:
            queue.put((x,y-1))

        if y < pixel_region.h-1 and pixel_region[x,y+1][3] != '\x00' and not visited[x,y+1]:
            queue.put((x,y+1))
...
register(
    ...
    [
        ...
        (PF_DIRNAME, 'save-path', 'Path for HTML export', os.getcwd()),
        (PF_STRING, 'base-name', 'Base name for assets', ''),
    ],
    [],
    cut_assets,
